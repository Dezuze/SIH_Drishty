    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 text-slate-700 hover:text-emerald-400 text-sm font-bold bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm hover:border-emerald-500/50 transition-colors"
          >
        ...
        <div className="bg-white/90 rounded-3xl border border-slate-200 shadow-2xl overflow-hidden backdrop-blur-md">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-8">
            
            {/* Left Column: Large Image */}
            <div className="lg:col-span-6 p-6 sm:p-8 bg-slate-50/60 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-slate-200">
              <div className="relative aspect-4/3 rounded-2xl overflow-hidden bg-slate-50 border border-slate-200 shadow-inner group">
                <img
                  src={product.image}
                  alt={product.name}
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  ...
                  {product.harvestDate && (
                    <span className="bg-slate-50/85 text-emerald-300 text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-xs flex items-center gap-1 border border-slate-200">
                ...
                {/* Stock Tag on Image */}
                <div className="absolute bottom-4 right-4 bg-slate-50/90 text-slate-900 text-xs font-black px-3 py-1.5 rounded-xl shadow-md border border-slate-300 flex items-center gap-1.5">
            ...
            <div className="lg:col-span-6 p-6 sm:p-8 flex flex-col justify-between space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Price Display */}
                <div className="bg-slate-50/80 border border-slate-200 p-4 rounded-2xl flex items-baseline justify-between">
                  <div>
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-600 block">Unit Price</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl sm:text-4xl font-black text-emerald-400 font-mono">₹{product.price}</span>
                      <span className="text-base font-bold text-slate-600">/ {product.unit}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-semibold text-slate-600 block">Farm Stock</span>
                    <span className="text-sm font-black text-emerald-300 bg-white px-2.5 py-1 rounded-lg border border-emerald-500/40 inline-block mt-0.5 font-mono">
                ...
                <div className="bg-slate-50/90 border border-slate-200 rounded-2xl p-4 space-y-2">
                  ...
                  {product.farmStory && (
                    <p className="text-xs text-slate-700 italic pt-1 border-t border-slate-200/80">
                      "{product.farmStory}"
                    </p>
                  )}
                </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-200">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="inline-flex items-center border border-slate-300 rounded-2xl bg-slate-50 overflow-hidden shadow-xs">
                      <button
                        onClick={handleDecrement}
                        disabled={quantity <= 1}
                        className="p-3 text-slate-700 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                      ...
                      <input
                        ...
                        className="w-16 text-center bg-white py-2 font-black text-slate-900 focus:outline-hidden border-x border-slate-300 text-base font-mono"
                      />
                      <button
                    ...
                    <div className="flex-1 bg-emerald-950/80 border border-emerald-500/40 rounded-2xl px-4 py-2.5 text-right">
                      <span className="text-[11px] font-semibold text-slate-600 block">Total Calculated Price</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  ...
                  <button
                    ...
                    disabled={product.available === 0}
                    className="py-3.5 px-6 rounded-2xl font-bold text-sm bg-slate-100 hover:bg-slate-200 text-slate-900 border border-slate-300 flex items-center justify-center gap-2 shadow-md hover:shadow-lg transition-all cursor-pointer"
                  >
