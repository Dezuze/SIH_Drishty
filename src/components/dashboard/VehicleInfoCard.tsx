  ...
  return (
    <div className="rounded-3xl bg-white/90 border border-slate-200 p-5 shadow-xl flex flex-col justify-between">
      ...
      <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 font-mono">
                Reg: <span className="text-slate-800 font-bold">{vehicle.regNumber}</span>
              </p>
            </div>
          </div>
        ...
        {/* Driver Profile */}
        <div className="my-3.5 p-3 rounded-2xl bg-slate-50/70 border border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h4 className="font-bold text-sm text-slate-900">{vehicle.driverName}</h4>
                <div className="flex items-center text-amber-400 text-xs font-bold">
                  <Star className="w-3 h-3 fill-current ml-0.5" />
                  <span className="ml-0.5">{vehicle.driverRating}</span>
                </div>
              </div>
              <p className="text-[11px] text-slate-600">Certified Cold-Chain Agri Pilot</p>
            </div>
          </div>

          <a 
            href={`tel:${vehicle.driverPhone}`}
            className="p-2 rounded-xl bg-white hover:bg-slate-100 text-emerald-400 border border-slate-300 transition"
            title="Call Driver"
          >
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          {/* Battery Status */}
          <div className="p-3 rounded-2xl bg-slate-50/60 border border-slate-200/80">
            ...
            <div className="w-full bg-white rounded-full h-1.5 p-0.5 overflow-hidden">
              <div className="bg-emerald-400 h-full rounded-full" style={{ width: `${vehicle.batteryPercent}%` }} />
            </div>
            <span className="text-[10px] text-slate-600 mt-1 block">Est. 112 km Range Left</span>
          </div>
      ...
      <div className="mt-3.5 pt-3 border-t border-slate-200 flex items-center justify-between text-xs text-slate-600">
        <span className="flex items-center gap-1">
          ...
          Speed: <strong className="text-slate-900 font-mono">{isSimulating ? '38 km/h' : '0 km/h (At Depot)'}</strong>
        </span>
